...
// KCU105 Hardware Golden top for the original pcie_phy_0_ex reference flow.
// phy_bringup_seq is the only block that generates the six board.v controls.
// The original Xilinx phy_ctrl.v in the wrapper remains the sole producer of
// PHY_RATE, PHY_POWERDOWN, TXEQ, as_mac_in_detect, and as_cdr_hold_req.
module kcu105_pcie_phy_bringup_top #(
    ...
    // board.v waits for sys_rst_n deassertion, then for the PHY-ready LED.
    // In hardware these are PERST# and the PHY status-reset release.
    phy_bringup_seq #(
    ...
    // LEDs expose the original phy_ctrl state and the sequence progress.
