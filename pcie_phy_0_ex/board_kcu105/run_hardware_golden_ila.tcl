# Program and capture the 14aba94-based Hardware Golden ILA.
# Usage:
#   vivado -mode batch -source run_hardware_golden_ila.tcl \
...
if {$action ni {program-arm capture-wait upload status}} {
    error "action must be program-arm, capture-wait, upload, or status"
}
...
if {$action eq "program-arm"} {
    set trigger [get_hw_probes -of_objects $ila -filter {NAME =~ "*gen3_request*"}]
    if {[llength $trigger] != 1} { error "gen3_request probe not found" }
    set_property CONTROL.TRIGGER_POSITION 4096 $ila
    reset_hw_ila $ila
    set_property TRIGGER_COMPARE_VALUE eq1'b1 [lindex $trigger 0]
    run_hw_ila $ila
    puts "PCIE_PHY_0_EX_HARDWARE_GOLDEN_ILA_ARM_PASS device=$device ila=$ila"
    ...
    puts "PCIE_PHY_0_EX_HARDWARE_GOLDEN_ILA_CAPTURE_PASS capture_dir=$capture_dir"
} elseif {$action eq "upload"} {
    set data [upload_hw_ila_data $ila]
    set stamp [clock format [clock seconds] -format {%Y%m%d_%H%M%S}]
    write_hw_ila_data -force -csv_file [file join $capture_dir ${stamp}_phy.csv] $data
    write_hw_ila_data -force [file join $capture_dir ${stamp}_phy.ila] $data
    puts "PCIE_PHY_0_EX_HARDWARE_GOLDEN_ILA_UPLOAD_PASS capture_dir=$capture_dir"
} else {
    puts "PCIE_PHY_0_EX_HARDWARE_GOLDEN_ILA_STATUS_PASS device=$device ila=$ila"
}
