...
set build_dir [file join $script_dir build_k00_m02]
...
read_xdc [file join $script_dir k00_m02_async_pkt_fifo.xdc]

synth_design -top pcie_async_pkt_fifo -part $part_name -mode out_of_context \
    -generic LGFIFO=9

# afifo.v 已标记跨域移位级，最终接收级属性由约束层补齐，第三方源码保持不变。
...
if {[llength $afifo_final_sync_cells] != 40} {
    error "K00 M02 afifo 最终同步级数量异常：[llength $afifo_final_sync_cells]，期望 40"
}
set_property ASYNC_REG TRUE $afifo_final_sync_cells
set_property SHREG_EXTRACT NO $afifo_final_sync_cells

write_checkpoint -force [file join $build_dir k00_m02_synth.dcp]
...
if {$total_bram_equivalent < 2} {
    error "K00 M02 BRAM 推断失败：RAMB36=$ramb36_count RAMB18=$ramb18_count"
}
if {$async_reg_count < 40} {
    error "K00 M02 CDC 属性检查失败：ASYNC_REG=$async_reg_count，期望至少 40"
}

set summary_path [file join $build_dir summary.txt]
set summary_file [open $summary_path w]
puts $summary_file "K00_M02_VIVADO_PASS"
...
puts "K00_M02_VIVADO_PASS part=$part_name RAMB36=$ramb36_count RAMB18=$ramb18_count ASYNC_REG=$async_reg_count"
