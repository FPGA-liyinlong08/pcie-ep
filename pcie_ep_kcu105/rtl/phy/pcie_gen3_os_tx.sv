...
// EIEOS and inserts another EIEOS after every 32 TS blocks. SDS is deliberately
// not emitted here: it belongs to Configuration.Idle/Recovery.Idle when a data
// stream starts, not to Recovery.RcvrLock/RcvrCfg. Symbols 1..15 of each TS are
// scrambled while the 1E/2D block identifier remains clear; the LFSR still
// advances over it and is re-seeded after each EIEOS.
module pcie_gen3_os_tx (
    ...
    localparam       SEND_EIEOS = 1'b0;
    localparam       SEND_TS    = 1'b1;
    ...
    always @* begin
        ...
        if (stream_state == SEND_EIEOS) begin
            ...
            case (active_index)
                ...
                // Symbols 6..9 carry the phase-0 EQ preset/coefficient tuple.
                // Preset 4 is the PHY reset default and is legal for lane 0.
                2'd1: plain_data = {8'h28, 8'h01,
